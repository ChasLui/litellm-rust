...
mod tests {
    ...
    fn directive_parses_no_cache_and_no_store() {
        ...
        assert_eq!(read_directive(&h, &body), CacheDirective { read: true, store: true });
        h.insert(CACHE_CONTROL, "no-store".parse().unwrap());
        assert_eq!(read_directive(&h, &body), CacheDirective { read: true, store: false });
        h.insert(CACHE_CONTROL, "no-cache".parse().unwrap());
        assert_eq!(read_directive(&h, &body), CacheDirective { read: false, store: false });
    }
    ...
    fn directive_honours_body_cache_param() {
        ...
        assert_eq!(read_directive(&h, &no_cache), CacheDirective { read: false, store: false });
        let no_store = json!({"cache": {"no-store": true}});
        assert_eq!(read_directive(&h, &no_store), CacheDirective { read: true, store: false });
        // No `cache` param leaves the default behaviour intact.
        assert_eq!(read_directive(&h, &json!({"model": "x"})), CacheDirective { read: true, store: true });
    }
